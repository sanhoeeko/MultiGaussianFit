...
class KMeans:
    ...
    def fit(self, X, weight):
        ...
        self.centroids = X[np.random.choice(
            range(len(X)), self.n_clusters, replace=False)]
            ...
            for cluster in clusters:
                if cluster:
                    ...
                    self.centroids.append(
                        X[np.random.choice(range(len(X)), 1)][0])
...
class FitResult:
    def __init__(self, res, cov):
        self.coefs = res
        # it is not a valid value, but it reflects the uncertainty
        self.uncertain = np.max(cov)
        if self.uncertain > 1:
...
def multi_gaussian_fit(x, y, peaks):
    ...
    print(centers)
    p0 = np.ones((peaks, 3))  # Import necessary libraries
...
def multi_gaussian_fit(x, y, peaks):
    ...
    p0[:, 2] *= 0.1  # too large or too small initial sigma will cause crash
    p0 = p0.reshape(-1)
